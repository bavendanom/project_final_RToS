...
const Servo mi_servo = {
    .channel = { .gpio_num = 18, .channel = LEDC_CHANNEL_0 },
    .config = {
        .mode = LEDC_LOW_SPEED_MODE,
        .timer = LEDC_TIMER_0,
        .duty_res = LEDC_TIMER_12_BIT,
        .frequency = 50,
        .invert = 0} // Usa la variable definida arriba
};
...
void servo_init(void) {
    pwm_channel_init(&mi_servo.config, &mi_servo.channel);
    ESP_LOGI(TAG, "Servo inicializado en GPIO %d", mi_servo.channel.gpio_num); // Corregir log
}
